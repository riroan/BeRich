...
## 보류 — HA 전략이 미확정(형성 중) 일봉으로 초기화될 수 있음 (2026-08-17)

**증상:** `HeikinAshiFlipStrategy.initialize()`가 브로커 히스토리를 전부 확정봉으로 취급하는데,
`get_historical_bars`는 오늘 형성 중인 봉을 걸러내지 않는다 (yfinance `history(interval="1d")`는
장중에 오늘 행 포함). 미확정 봉의 부분 OHLC로 계산한 HA 색이 가짜 전환을 만들어 잘못된 매매가
나갈 수 있다.

**재현:** 25봉 평탄 + 마지막에 "장중 스냅샷"(97/115/97/114)으로 `initialize()` → `ha_flip_up` 발생.
마감 후 실제 확정값(97/115/90/92)을 `confirm_daily_bar`로 반영하면 색은 **음봉** — 매수가 오판이었음.

**발동 조건 (좁음):**
- 핫리로드는 *변경된* 전략만 재초기화한다 (`_strategy_unchanged`면 인스턴스 재사용) →
  장중에 HA 전략의 파라미터/심볼을 편집한 경우에만 해당
- 또는 **페이퍼 모드 장중 재시작** — `paper: warmup_hours = 0`이라 워밍업 게이트가 없음

**실측 빈도 (6종목 × 60일, 1시간봉으로 부분봉 재구성):**

| 장중 경과 | HA 색 불일치 | 그중 가짜 *전환*까지 만드는 경우 |
|---|---|---|
| 25% | 8.3% | 1.9% |
| 50% | 5.8% | 2.2% |
| 75% | 3.1% | 0.6% |

→ 리로드가 일어난 날에만 1~2%. 실질 발생은 연 단위.

**꼬리 위험:** 엣지 트리거라 잘못 진입 후 마감에 색이 정정되면 매도 조건
(`bull[i-1] 음 && bull[i-2] 양`)이 성립하지 않아 다음 사이클까지 청산 못 함.

**수정 방안 (권장순):**
1. `initialize()`에서 마지막 봉의 날짜가 "현재 거래일"이면 버린다. 정확하지만 KST↔미국장
   거래일 판정 헬퍼가 필요 (`get_current_session` 내부 `us_date`만 있고 노출 안 됨).
2. 확정 폴링이 1회 돌기 전엔 매매 금지 — 타임존 무관하지만 장중 리로드 후 **청산도 막힘**.
3. 시드된 마지막 봉을 확정 전까지 전환 판정에서 제외 — 청산은 가능하나 하루 묵은 전환에 매매.

**판단:** 보류. HA는 현재 차트/백테스트 용도로만 쓰고 실매매에 안 올림. **실전 투입 전에 1번으로
수정할 것.** 그때까지 우회책: HA 전략 설정 편집은 장 마감 후(07:00~09:00 KST, CLOSED 구간)에 하면
틱이 안 돌아 신호가 나가지 않는다.

**관련:** 같은 codex 리뷰에서 나온 나머지 2건은 수정 완료 —
핫리로드 시 HA 포지션 유실(`data_loader.py` 복원 게이트가 RSI 사다리 속성을 요구), 백테스트
워밍업 경계에서 전환 유실(`backtest_ha.py` 색 배열을 절단 후 프레임에서 생성).
